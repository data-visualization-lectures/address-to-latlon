...
ビルド済みの `out/` ディレクトリ一式を静的ホスティングにアップロードするだけで公開できます。以下のような手段が利用可能です。

- **GitHub Pages**: `out/` 内のファイルを `gh-pages` ブランチに配置
- **Netlify / Vercel**: ビルドコマンドを `npm run build`、公開ディレクトリを `out` に設定
- **任意のオブジェクトストレージ / CDN**: `out/` をそのまま同期

継続的デプロイを設定する場合は、CI で `npm ci && npm run build` を実行し、生成された `out/` をアーティファクトとしてアップロードしてください。
